...
version=$(echo "$drvname" | sed -e "s|NVIDIA\-Linux\-${arch}\-||" -e 's|\.run$||' -e 's|\-grid$||' -e 's|\.tar\..*||' -e 's|nvidia\-xconfig\-||')
drvbranch=$(echo "$version" | awk -F "." '{print $1}')

tarball="nvidia-xconfig-${version}"
...
fetch_input() {
    ...
    if [[ ! -f "$inputfile" ]]; then
        dep wget
        kmd wget "https://download.nvidia.com/XFree86/nvidia-xconfig/${tarball}.tar.bz2" -O "$inputfile"
    fi
}
...
build_rpm()
{
    ...
    cp -v -- *xconfig*.tar* "$topdir/SOURCES/"
...
build_triple_rpm()
{
    ...
    if [[ -f "nvidia-xconfig-${version}.tar.gz" ]]; then
        extension="gz"
    elif [[ -f "nvidia-xconfig-${version}.tar.bz2" ]]; then
        extension="bz2"
    fi
